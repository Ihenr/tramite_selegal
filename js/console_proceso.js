...
function listar_proceso() {
  tbl_area = $("#tabla_proceso").DataTable({
    ...
    processing: true,
    ajax: {
      url: "../controller/proceso/controlador_listar_proceso.php",
      type: "POST",
    },
    columns: [
      { defaultContent: "" },
...
function Cargar_Select_Institucion() {
  $.ajax({
    url: "../controller/proceso/controlador_cargar_select_institucion.php",
    type: "POST",
  }).done(function (resp) {
    let data = JSON.parse(resp);
    if (data.length > 0) {
      let cadena = "<option value=''> SELECCIONAR LA INSTITUCION</option>";
      for (let i = 0; i < data.length; i++) {
        cadena +=
          "<option value='" + data[i][0] + "'>" + data[i][1] + "</option>";
      }
      document.getElementById("select_institucion").innerHTML = cadena;
    } else {
      cadena += "<option value=''>NO HAY INSTITUCIONES</option>";
      document.getElementById("select_institucion").innerHTML = cadena;
    }
  });
}
...
function listar_proceso_activos() {
  tbl_area = $("#tabla_proceso").DataTable({
    ordering: false,
    bLengthChange: true,
    searching: { regex: false },
    ...
    processing: true,
    ajax: {
      url: "../controller/proceso/controlador_listar_proceso_activo.php",
      type: "POST",
    },
    columns: [
      { defaultContent: "" },
...
var tbl_procesos_finalizados;
function listar_proceso_finalizados() {
  tbl_procesos_finalizados = $("#tabla_proceso").DataTable({
    ordering: false,
    bLengthChange: true,
    searching: { regex: false },
    lengthMenu: [
    ...
    processing: true,
    ajax: {
      url: "../controller/proceso/controlador_listar_proceso_finalizados.php",
      type: "POST",
    },
    columns: [
      { defaultContent: "" },
      ...
      // { data: "pro_abono" },
      {
        ...
        render: function (data, type, row) {
          return (
            "$" +
            data +
            "<br><button class='abonofin btn btn-success btn-xs'><i class='fas fa-list-ul'></i></button>"
          ); // Agrega el símbolo de dólar al principio del valor
        },
      },
      {
        ...
        render: function (data, type, row) {
          return (
            "$" +
            data +
            " <br> <button class='extrasfin btn btn-success btn-xs'><i class='fas fa-list-ul'></i></button>"
          ); // Agrega el símbolo de dólar al principio del valor
        },
      },
      {
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (data == "INACTIVO") {
            return "&nbsp &nbsp <button class='masfin btn btn-danger btn-xs'><i class='fa fa-plus'> </i></button> <span class='badge bg-danger' style='font-size: 10px;'>FINALIZADO</span>";
          }
        },
      },
    ],
  ...
  tbl_procesos_finalizados.on("draw.td", function () {
    var PageInfo = $("#tabla_proceso").DataTable().page.info();
    tbl_procesos_finalizados
      .column(0, { page: "current" })
      .nodes()
      .each(function (cell, i) {
        cell.innerHTML = i + 1 + PageInfo.start;
...
$("#tabla_proceso").on("click", ".masfin", function () {
  var data = tbl_procesos_finalizados.row($(this).parents("tr")).data();
  if (tbl_procesos_finalizados.row(this).child.isShown()) {
    data = tbl_procesos_finalizados.row(this).data(); // permite llevar los datos cuando es tamaño celular y usar el responsive de dataTables
  }
  $("#modal_mas").modal("show");
  document.getElementById("txt_id_pro").value = data.pro_institucion;
  document.getElementById("txt_nom").value = data.pro_cliente;
...
$("#tabla_proceso").on("click", ".abonofin", function () {
  var data = tbl_procesos_finalizados.row($(this).parents("tr")).data(); //en tamaño de escritorio
  if (tbl_procesos_finalizados.row(this).child.isShown()) {
    data = tbl_procesos_finalizados.row(this).data(); // permite llevar los datos cuando es tamaño celular y usar el responsive de dataTables
  }
  $("#modal_abono").modal("show");
...
$("#tabla_proceso").on("click", ".extrasfin", function () {
  var data = tbl_procesos_finalizados.row($(this).parents("tr")).data(); //en tamaño de escritorio
  if (tbl_procesos_finalizados.row(this).child.isShown()) {
    data = tbl_procesos_finalizados.row(this).data(); // permite llevar los datos cuando es tamaño celular y usar el responsive de dataTables
  }
